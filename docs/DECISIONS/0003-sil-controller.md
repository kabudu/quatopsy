...
`execution` may be `sil`, `pil`, or `hil`. `pil` isolates the control cycle in a child process on the host CPU. `hil` sends commands to a loopback actuator-emulator process. Optional declared plant models (command-to-torque lag named `wheel_lag_s`, magnetic residual against a frozen inertial field, gravity-gradient against a frozen nadir) and sensor models (star-tracker delay, gyro ARW) are software only. `star_tracker_delay_s: 0` means zero attitude delay. `latency_class` is `bounded-software` only. `hardware.class: physical` is refused. The command writes CSV, a declared manifest, and `quatopsy.control/1` with no report `result`. `analyze` remains the only verdict owner.
