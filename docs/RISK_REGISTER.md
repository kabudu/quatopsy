...
| R9 | Performance target fails on realistic profiles | Low | Medium | Performance | Local CI million-sample miss | Contiguous data, streaming parse, bounded reports, release-profile assertion | Publish a lower supported limit or optimise the measured hotspot |
| R10 | Name or mark conflicts with a relevant product | Low | High | Legal | Similarity or trademark review finding | Point-in-time search; private visibility until legal review | Rename before public productisation |
...
| R19 | A feasible plan is treated as certified or globally optimal | Medium | High | Product | Plan status or copy implies approval | `optimality_class` not-claimed, no `result` field, kernel-owned analyze | Suspend planner claims and keep diagnostic boundary |
