...
Inputs may be malicious, corrupted, ambiguous, extremely large, numerically adversarial, or crafted to flood findings. An adapter may misdeclare frames or units. A planner problem may declare an oversimplified model. A controller problem may declare an envelope the plant cannot honour. A dependency or cache may be compromised. A user may mistake advisory output, a feasible plan, or a tracked closed-loop candidate for flight approval. A future integration may expose credentials or reinterpret verdicts.
...
Archives, ROS bags, MCAP, and SPICE files are handled only by isolated adapters with record, message, compression, and size limits. Compressed MCAP chunks and nested chunks are refused. SPICE reading is limited to little-endian IEEE DAF/CK type 3 discrete pointing. These formats do not enter core CSV parsing. Planner and controller problems are bounded JSON with unknown fields refused; generated trajectories enter the kernel only as ordinary CSV plus manifest. The controller never opens a device. Physical `hardware.class` is refused.
